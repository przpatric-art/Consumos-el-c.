...
st.set_page_config(page_title="Generador de Boletas Electricas", page_icon="⚡")

# --- MENÚ LATERAL (SIDEBAR) ---
st.sidebar.header("📋 DATOS DEL CLIENTE")
nombre = st.sidebar.text_input("Nombre y Apellido", "Juan Perez")
n_cliente = st.sidebar.text_input("Numero de Cliente", "001")

st.sidebar.header("💰 VALORES DE CALCULO (INTERNOS)")
...
cobro_porton_camara = st.sidebar.number_input("Cobro por Porton y Camara ($)", min_value=0, value=0)
...
# --- INTERFAZ PRINCIPAL ---
st.title("⚡ Generador de Boleta Electrico")

st.subheader("Resumen de Cobro Actual")
c1, c2, c3, c4 = st.columns(4)
c1.metric("Consumo", f"{consumo_mes} kWh")
c2.metric("Porton/Camara", format_clp(cobro_porton_camara))
...
def generar_imagen_final():
    ...
    # Textos sin acentos para evitar errores de renderizado
...
st.image(img_boleta, caption="Vista previa de la boleta (Sin acentos para evitar errores)")
